...
The Inbox can process links or text that the user pastes or shares from another app. Processing happens on the device. Each capture can be written as an independent Markdown document to a folder explicitly selected by the user.
...
Eligible new subscribers can start a seven-day free trial through Apple's purchase sheet. The monthly subscription renews automatically after the trial unless canceled beforehand in the Apple Account. Apple manages eligibility and renewal. Older versions stored free-usage counters in the device Keychain; this version no longer uses those counters to limit recording or capture. The developer cannot access that Keychain data.
...
言续不收集个人数据，不创建账号，不展示广告，不使用第三方统计 SDK，也不会将录音或收集内容发送到开发者运营的服务器。

## 麦克风访问

获得用户授权后，言续只会在用户主动开始录音时使用麦克风。音频会写入 App 本地容器；用户主动选择时，也可以写入通过 iOS 文件选择器授权的文件夹。开发者无法访问这些录音。

## 链接、文字与分享扩展

Inbox 可以处理用户粘贴或从其他 App 分享的链接与文字。处理过程在设备本地完成。每次收集都可以作为独立 Markdown 文档写入用户明确选择的文件夹。
...
符合条件的新订阅用户可通过 Apple 的购买界面开始 7 天免费试用。试用结束后，月度订阅会自动续订，除非用户提前在 Apple 账户中取消。订阅资格和续订由 Apple 管理。旧版本曾在设备钥匙串中保存免费用量计数；新版不再以该计数限制录音或收集，开发者无法访问这些钥匙串数据。

## 实时活动与快捷指令

言续使用 Apple 实时活动与快捷指令显示录音状态，并支持用户主动触发开始和停止录音。这些功能不会将录音内容传输给开发者。

## 第三方

言续不会出售、出租或分享用户数据。如果用户主动选择在其他 App 中打开文档，或将文件保存到第三方文件提供商，该操作由用户发起，并受所选服务自身隐私政策约束。
